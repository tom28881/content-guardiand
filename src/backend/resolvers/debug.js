...
import { runRealScan } from '../services/scan';
...
async function testScanDirectly() {
  ...
  try {
    ...
    const { getPagesBatchV2WithRetry } = await import('../services/confluence.js');
    ...
    console.log('🚀 Testing runRealScan directly...');
...
export function registerDebugResolvers(resolver) {
  resolver.define('debugScan', async () => {
    console.warn('[resolver.debugScan] starting direct scan test...');
    try {
      await testScanDirectly();
      return { ok: true, message: 'Debug scan completed - check logs' };
    } catch (error) {
      console.error('[resolver.debugScan] error:', error);
